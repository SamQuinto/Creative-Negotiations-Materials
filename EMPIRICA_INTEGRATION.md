...
The researcher's Empirica screenshot confirmed inline italics, payoff colors and the indented example render correctly. Bullet and numbered paragraphs now use an explicit block `span` with `margin-left:1.5em; padding-left:1.25em; text-indent:-1.25em` to indent the marker and align wrapped lines under the text. This does not depend on the host's list styles. This latest indentation still needs a visual check in Empirica after reloading.

If tags appear literally or colors are removed, check the reader's formatting support; **do not disable sanitization or enable arbitrary HTML**. Only the generated `em`, `strong`, `u` and indented `span` formatting and two fixed colors (`#166534`, `#b91c1c`) are needed. No script, external asset or event handler is included in the narrative. The previous `role_markdown` function remains available for a bold-only fallback; apply the review wording edits too if using it. After an update, reload the role JSON; already-open sessions or pinned older commit URLs may still show the old text.
